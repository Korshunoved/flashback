...
if (!empty( $_POST['cont']))
{
update_user_meta( $idi, 'cont', $_POST['cont'] );	
}
if (!empty( $_POST['goro']))
{
update_user_meta( $idi, 'goro', $_POST['goro'] );
}
if (!empty( $_POST['idu']))
{
update_user_meta( $idi, 'idu', $_POST['idu'] );
}
	
	
if (!empty( $_POST['pho']))
{
update_user_meta( $idi, 'pho', $_POST['pho'] );	
}
if (!empty( $_POST['inta']))
{
update_user_meta( $idi, 'inta', $_POST['inta'] );
}
...
$data=date('d-m-Y');$all=array();$all1=array();
$ter=strtotime($data);
$today=array();
$fut=array();
$past=array();
...
/////////////////////////ticketbuy
$sbor=array();
$folo=get_user_meta($current_user->ID, 'buytik', true);
$fol=explode(';',$folo);
foreach ($fol as $pop)
{
...
foreach ($sbor as $nov1)
{
	
	$nov = get_post_meta($nov1, '_number_tov', true);
 $dat=get_post_meta($nov, '_text_dat', true);
	 $er=explode('.',$dat);
	 $tu=$dat;
    $tert=strtotime($tu);	
if ($data==$dat)
...
echo $fio=get_user_meta( $idd, 'fio', true);
		...
		<li class="nav-item">
            <a class="nav-link " data-toggle="tab" href="#tab-sidebar-0" role="tab" aria-controls="tab-sidebar-1" aria-selected="false" >My Events</a></li>

		
          <li class="nav-item">
            <a class="nav-link " data-toggle="tab" href="#tab-sidebar-1" role="tab" aria-controls="tab-sidebar-1" aria-selected="false" >Manage Profile</a></li>
          <li class="nav-item">
            <a class="nav-link " data-toggle="tab" href="#tab-sidebar-2" role="tab" aria-controls="tab-sidebar-2" aria-selected="false" >Contact Info</a></li>
          <li class="nav-item ">
            <a class="nav-link active" data-toggle="tab" href="#tab-sidebar-3" role="tab" aria-controls="tab-sidebar-3" aria-selected="false" >FlashBs</a></li>
        
          <!--<li class="nav-item"><a class="nav-link " data-toggle="tab" href="#tab-sidebar-5" role="tab" aria-controls="tab-sidebar-5" aria-selected="false" >Manage Event</a></li>-->
          <li class="nav-item"><a class="nav-link " data-toggle="tab" href="#tab-sidebar-6" role="tab" aria-controls="tab-sidebar-6" aria-selected="false" >Withdraw</a></li>
...
border-radius: 30px;" class="nav-item verify-ticket">
		<a style="color: #141414;" class="nav-link " href="#">Become an Influencer</a></li> -->
    <li class="nav-item nav-item-btn">
            <a href="#" class="btn btn_green">Become an Influencer</a>
          </li>
      ...
      <article class="content col content-tabs">
        <div class="tab-content" id="myTabContentSidebar">
          ...
          <div class="tab-pane fade" id="tab-sidebar-0" role="tabpanel">
         ...
         <div class="tab-content" id="myTabContent">
          <div class="tab-pane fade show active" id="tab-1" role="tabpanel" style="padding-bottom: 30px;">
		  ...
		  ?>
            <div class="your-event">
              <div style="cursor:pointer"  onclick="window.location.href='/addevent/?id=<?=$prod?>'" class="img">
                <div class="img-in">
                  <img src="<? echo  get_the_post_thumbnail_url( $prod, 'thumbnail' );?>" alt="" style="height: 240px;">
                </div>
              ...
              <div class="desc desc_qrcode">
                ...
                <div class="qrcode">
                 		<?
$urod='http://api.qrserver.com/v1/create-qr-code/?size=150x150&data=http://cryptotik.beget.tech/user/?id='.$idd;
		?>
		<img src="<?=$urod?>">
                </div>
...
echo $fio=get_user_meta( $current_user->ID, 'fio', true);
		?></div>
                    ...
                    <div class="item">
                      ...
                      <div class="h4"><?=$user->user_email?></div>
                    </div>
                  </div>
                  ...
                  <div class="item">
                    ...
                    <ul class="list">
                      <li>
                        <input name="cont" value="<? echo get_user_meta( $current_user->ID, 'cont', true);?>" type="text" placeholder="Country">
                      </li>
                      <li>
                        <input type="text" name="goro" value="<? echo get_user_meta( $current_user->ID, 'goro', true);?>" placeholder="City">
						
                      </li>
                    </ul>
                  </div>
                  <div class="item">
                    <div class="h3">Set User ID</div>
                    <input name="idu" value="<? echo get_user_meta( $current_user->ID, 'idu', true);?>" type="text">
					         <p class="trext sm">This is your unique ID  that you’ll use for ticket transfers and other transactions within the FlashBack platform</p>
                  </div>
                </li>
                ...
                <li>
                  <div class="item">
                    ...
                    <input name="pho" value="<? echo get_user_meta( $current_user->ID, 'pho', true);?>" type="text" placeholder="+">
                  </div>
                  <div class="item item-size">
                    <div class="h3">Intrests</div>
                    <input name="inta" value="<? echo get_user_meta( $current_user->ID, 'inta', true);?>" type="text" placeholder="">
                    <div class="size">e.g #LGBTQ ; #NEWYORK ; #LATIN</div>
                  </div>
<ul id="uploadImagesList">
        <li class="item template">
            ...
            <span class="delete-link" style="display:none" title="Удалить">Удалить</span>
                ...
                <input type="file"  onclick="$('.delete-link').click();$('#ray2').text('Profile Picture');"  name="file6" id="input_logo" class="inputfile"
                       data-multiple-caption="{count} files selected" multiple="">
                <label for="input_logo">
                  <strong id="rao1" style="margin-top: 20px;">Upload</strong>
                  <span style="font-size:14px;" id="ray2">Profile Picture</span>
                </label>
                <div class="size">200px X 200px</div>
              </div>
        ...
        <li class="item template">
            ...
            <span class="delete-link1" style="display:none" title="Удалить">Удалить</span>
                 ...
                 <div class="inputfile-box">
                <input type="file" onclick="$('.delete-link1').click();$('#ray1').text('Banner');" name="file7" id="input_bnner" class="inputfile"
                       data-multiple-caption="{count} files selected" multiple="">
                <label for="input_bnner">
                  <strong id="rao2" style="margin-top: 20px;">Upload</strong>
                  <span style="font-size:14px;" id="ray1">Banner </span>
                </label>
                <div class="size">820px X 312px</div>
              </div>
<div>
          <button class="btn btn_green" name="sub" value="sub" type="Submit" style="margin-top: 60px;font-weight: bold;">Submit</button>
        </div>
                </li>
              </ul>
            </div>
...
echo $cont=get_user_meta( $current_user->ID, 'cont', true);
		?>, <?
		
echo $goro=get_user_meta( $current_user->ID, 'goro', true);
		?></div>
                    </div>
                  </div>
                </li>
          ...
          <div class="tab-pane fade show active" id="tab-sidebar-3" role="tabpanel">
		  
		  
         <ul class="tabs nav nav-tabs nav-tabs-white" id="myTab" role="tablist">
          <li class="nav-item">
            <a class="nav-link active" data-toggle="tab" href="#tab-11" role="tab"
               aria-controls="tab-1" aria-selected="false">Today <span><?=count($today1)?></span></a>
          </li>
          <li class="nav-item">
            <a class="nav-link " data-toggle="tab" href="#tab-21" role="tab"
               aria-controls="tab-2" aria-selected="false">Upcoming  <span><?=count($fut1)?></span></a>
          </li>
          <li class="nav-item">
            <a class="nav-link " data-toggle="tab" href="#tab-31" role="tab"
               aria-controls="tab-3" aria-selected="false">Past <span><?=count($past1)?></span></a>
          </li>
          <li class="nav-item">
         ...
         <div class="tab-content" id="myTabContent">
		  ...
		  <?
		  foreach ($today1 as $prod)
		  {

$post = get_post($prod);
$nov1 = get_post_meta($prod, '_number_tov', true);
 $dat=get_post_meta( $nov1, '_text_dat', true);
	 ...
	 $tu=$er[0].'.'.$er[1];
		  ...
		  ?>
            <div class="your-event">
              <div  class="img">
                <div class="img-in">
                  <img src="<? echo  get_the_post_thumbnail_url( $prod, 'thumbnail' );?>" alt="" height="240">
                </div>
                <div class="h4"><? echo $post->post_title?> - <span><?=$tu?></span></div>
              </div>
              <div class="desc desc_qrcode">
 ...
 $dat=get_post_meta( $nov1, '_text_dat', true);
  ...
  $er=explode('-',$dat);
	 $tu=$er[0].'.'.$er[1];
          ...
          <div class="tab-pane fade" id="tab-sidebar-4" role="tabpanel">
            <div class="content-public-event">
              <ul class="events-list">
                <li>
                  <div class="img"><img src="/verstka/img/circle-profile/1.jpg" alt=""></div>
          ...
          <div class="tab-pane fade" id="tab-sidebar-5" role="tabpanel">Manage Event</div>
...
<script>
 ...
 $('#input_bnner').on('change', function () {
         var files = this.files;
